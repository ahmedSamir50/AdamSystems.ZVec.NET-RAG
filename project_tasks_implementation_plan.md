...
>
> Target Ecosystem: .NET 8 / 9 / 10 | Core Native Engine: ZVec.NET v1.0.0-beta.6 | License: Apache-2.0
...
- 📐 **`zvec-architect-strategy-expert`**: Product positioning ("No cloud, no Python, no kidding"), kill criteria monitoring, developer onboarding (`dotnet new zvec-rag`), enterprise vs OSS licensing, risk mitigation options with Pros/Cons.
...
ZVec.NET-RAG.slnx
 ...
 │    │         ├── Evaluation/ (IRagEvaluator, DeterministicEvaluator, SemanticTestEmbedder)
 │    └── ZVec.Rag.Template/
 │         └── ZVec.Rag.Template.csproj (dotnet new zvec-rag project template)
 └── tests/
...
- [x] **Story 2.4: Standalone `ZVec.Rag.Testing` Package & CI Fakes** (Owner: `zvec-rag-pipeline-expert`, `zvec-architect-strategy-expert`)
  - [x] **Task 2.4.1 (TDD)**: Create `src/ZVec.Rag.Testing/ZVec.Rag.Testing.csproj` NuGet package containing `DeterministicEmbedder` (hash-based pipeline unit tests) and `FakeChatClient`. **`SemanticTestEmbedder` / `IRagEvaluator` shipped in Story 2.8.**
...
- [x] **Story 2.7: RAG Pipeline Native AOT Gate (`ZVec.Rag.AotTestApp`)** (Owner: `zvec-native-aot-expert`, Reviewer: `zvec-rag-pipeline-expert`)
  ...
  - [x] **Task 2.7.2**: Isolate non-AOT packages (`ZVec.Rag.Pdf`, `ZVec.Rag.LLamaSharp`) behind `[RequiresUnreferencedCode]`; omit from `ZVec.Rag.AotTestApp`.
  - [x] **Task 2.7.3**: Verify `rag-aot-smoke` CI job passes on all 3 desktop RIDs (`linux-x64`, `win-x64`, `osx-x64`) with `TreatWarningsAsErrors` and zero trim/AOT warnings (`IL2026`, `IL3050`) in publish output. Harness runs `IngestTextAsync`, `RetrieveAsync`, and `AskAsync` (`FakeChatClient`).
  - **Acceptance Criteria**: Connector AOT (Story 0.2) remains closed; pipeline AOT verified on 3 desktop RIDs (text ingest + retrieve + Ask; no PDF/SSE/LLamaSharp).
- [x] **Story 2.8: RAG Evaluation Harness (`IRagEvaluator`)** (Owner: `zvec-rag-pipeline-expert`, Reviewer: `zvec-architect-strategy-expert`)
  - [x] **Task 2.8.1 (TDD)**: Implement `IRagEvaluator` in `ZVec.Rag.Testing` with Recall@K, MRR, nDCG on in-repo labeled fixtures (`tests/ZVec.Rag.Tests/Fixtures/`, ~50–200 Q/A pairs). Add `RecallAtKLift(baseline, treatment)` returning `{ Baseline, Treatment, Delta, RelativeLift }` with \(\mathrm{Lift}@K=(R_{new}-R_{base})/R_{base}\); if \(R_{base}=0\), `RelativeLift` is null and report \(\Delta\) only. Unit-test the formula. Optional gitignored BEIR download script for local SOTA checks — not shipped in the Testing NuGet. `SemanticTestEmbedder` for metric unit tests. Sample 03 INT8 gate = Recall@K ≥ 0.95 vs FP32 Flat on the **same** shipped fixture (not BEIR SOTA).
  - [x] **Task 2.8.2**: Add optional LLM-as-judge Faithfulness/Context Precision evaluators (off by default in CI); `DeterministicEvaluator` for CI.
  - **Acceptance Criteria**: Closes gap D-1; no RAGAS/LangSmith dependency.
- [x] **Story 2.9: Optional Section-Summary Helper (`GenerateSummaries`)** (Owner: `zvec-rag-pipeline-expert`, Reviewer: `zvec-security-expert`, `zvec-native-aot-expert`)
  - **Note:** implementation-plan **Story 2.9** ≠ project-plan Epic 2 checkbox **2.9** (citation tracking).
  - [x] **Task 2.9.1 (TDD)**: Add `IngestOptions.GenerateSummaries` (default `false`), `MaxSummaryTokens` (default `128`), `SummarySectionMaxTokens` (default `2048`). When enabled: split source into sections → `IChatClient` section summary (one call per section, channel stage before child embed) → upsert `ZVecRagSectionSummaryV1` into collection `rag_section_summaries` (own hybrid index, `embed(Summary)`); chunk section → upsert `ZVecRagRecordV1` into `rag_chunks` with `embed(Text)` unchanged and indexed `SectionSummaryId` FK. `ChunkId` formula unchanged. Re-ingest from scratch if the flag or section knobs change (document in README).
  - [x] **Task 2.9.2**: When enabled, `RagRetriever` runs **parallel hybrid** on both collections (union + parent boost: keep chunk hits; boost chunks whose parent summary also hit; add children of top-S matching summaries). `ContextPacker` prepends the section summary string; citations and pack body remain child `Text` only. Optional exclusive `SectionGate` (summary-first filter only) deferred post-v1 if tasked.
  - [x] **Task 2.9.3**: `IRagSecuritySanitizer` on source before summarize and on stored summary; trusted system policy (aboutness entailed by section + preserve verbatim IDs/numbers/names/dates/URLs/table cells). `ZVec.Rag.AotTestApp` keeps `GenerateSummaries = false` (no second collection in that smoke).
  - [x] **Task 2.9.4 (TDD)**: Contract test (not SOTA marketing): fixture where section text lacks query token Y, `FakeChatClient` summary states Y, child chunks lack Y; query Y must retrieve gold child `ChunkId` via summary collection path; assert with `SemanticTestEmbedder` / `IRagEvaluator`. Compare Lift@K vs `GenerateSummaries = false` baseline using Story 2.8 helper.
  - **Task 2.9.5 (optional, local)**: Real `IEmbeddingGenerator` paired Lift@K on same fixture — gitignored/local only; not README marketing or CI merge gate.
  - **Acceptance Criteria**: Closes gap D-9; default OFF; helps retrieve/cite accuracy — **not** Advanced RAG, not RAPTOR; no public Recall@K number until optional 2.9.5 run.
...
- [x] **Story 3.1: Project Template `ZVec.Rag.Template`** (Owner: `zvec-architect-strategy-expert`)
  - [x] **Task 3.1.1**: Create `dotnet new zvec-rag` template definitions (`template.json`, package `ZVec.Rag.Template`) with shortNames `zvec-rag`, `zvec-rag-aspnet`, `zvec-rag-maui` supporting Console, ASP.NET Core SSE, and MAUI Blazor Hybrid flags (`--llm`, `--embedder`, `--storage`). Include pre-embedded micro-fixture (100 pre-computed chunks) for instant 60s working onboarding. Note: Blazor WASM is explicitly excluded due to native C++ interop constraints. Do **not** use generic `rag` as primary shortName (ZVec-specific, not a generic RAG scaffold).
  - [x] **Task 3.1.2**: Package and test `ZVec.Rag.Template` NuGet.
- [x] **Story 3.2: Reference Sample Applications** (Owner: `zvec-rag-pipeline-expert`)
...
- [x] **Story 3.4: Honesty pass (Pdf magic, summary collection name, unified version, Arabic retrieve)** (Owner: `zvec-rag-pipeline-expert`, `zvec-ci-cd-expert`, `zvec-docs-expert`)
  - [x] **Task 3.4.1**: PDF magic-byte mismatch reject (`%PDF-`); `SummaryCollectionName` + convention; pin PdfPig 0.1.16; all `src/` packages `1.0.0-preview.1` via Directory.Build.props; Arabic fixture retrieve test; document versioning.
